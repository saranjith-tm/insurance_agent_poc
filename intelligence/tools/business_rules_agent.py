...
def _find(data: dict, *keys: str) -> Any:
    """Search nested dict for first key containing any of *keys (substring, case-insensitive)."""
...
def check_business_rules(extracted_data: dict, parsed_values: dict = None) -> dict:
    ...
    age = pv.get("age")
    sa = pv.get("sum_assured") or _parse_number(_find(extracted_data, "sum_assured"))
    h = pv.get("height_cm") or _parse_number(_find(extracted_data, "height"))
    w = pv.get("weight_kg") or _parse_number(_find(extracted_data, "weight"))
    loan_tenure = pv.get("loan_tenure_months") or _parse_number(_find(extracted_data, "loan_tenure", "tenure_months"))
    ...
    loan_raw = _find(extracted_data, "loan_amount")
    loan_amount = _parse_number(loan_raw)
    ...
    moratorium_raw = _find(extracted_data, "moratorium")
    ...
    # ── Rule 8: PPT Format ────────────────────────────────────────────────────
    ppt_raw = _find(extracted_data, "ppt", "premium_paying_term", "paying_term")
    if ppt_raw:
